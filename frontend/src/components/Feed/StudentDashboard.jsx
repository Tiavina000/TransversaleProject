      ...
      <div className="flex gap-3 justify-between">
        {[
          ...
          <div key={i} className="flex flex-col items-center">
            <span className="text-xl font-black text-[var(--text-primary)] leading-none">{String(t.v).padStart(2, '0')}</span>
...
function useCircularFeed(source, batchSize = 6) {
  ...
  const feed = (() => {
    ...
    const totalItems = batchSize * (loadCount + 1);
    return Array.from({ length: totalItems }, (_, i) => {
      const originalItem = source[i % source.length];
      return { ...originalItem, _feedKey: `feed-${i}-${originalItem.id}` };
    });
  })();
...
<div className="w-full h-full rounded-full flex items-center justify-center overflow-hidden" style={{ background: 'var(--bg-app)' }}>
                {item.auteur_image ? (
...
<div className="aspect-video flex items-center justify-center p-10 text-center" style={{ background: 'linear-gradient(135deg, color-mix(in srgb, var(--color-primary) 20%, transparent), var(--bg-app), color-mix(in srgb, var(--color-accent) 20%, transparent))' }}>
              <h4 className="text-xl sm:text-2xl font-black text-[var(--text-primary)] leading-tight drop-shadow-lg">{title}</h4>
          </div>
        )}
      </div>
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-5">
            <button onClick={() => setIsLiked(!isLiked)} className={`transition-all hover:scale-110 ${isLiked ? 'text-red-500' : 'text-[var(--text-primary)]/80 hover:text-red-500'}`}>
              <Heart size={26} fill={isLiked ? "currentColor" : "none"} />
            </button>
            <button className="text-[var(--text-primary)]/80 hover:text-primary transition-all hover:scale-110">
              <MessageCircle size={26} />
            </button>
            <button className="text-[var(--text-primary)]/80 hover:text-secondary transition-all hover:scale-110">
              <Share2 size={26} />
            </button>
          </div>
          <button onClick={() => setIsBookmarked(!isBookmarked)} className={`transition-all hover:scale-110 ${isBookmarked ? 'text-amber-400' : 'text-[var(--text-primary)]/80 hover:text-amber-400'}`}>
            ...
            <div className="glass p-8 rounded-[2.5rem] space-y-6">
              <div className="flex flex-col items-center text-center space-y-4">
                <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-primary to-secondary p-1.5 shadow-2xl shadow-primary/30">
                  <div className="w-full h-full rounded-full flex items-center justify-center text-4xl font-black text-[var(--text-primary)]" style={{ background: 'var(--bg-app)' }}>
              ...
              <div className="pt-6 border-t border-white/5 space-y-4">
                {displayStats.map((s, i) => (
                  <div key={i} className="flex items-center justify-between text-xs">
                    <span className="text-[var(--text-muted)] font-bold uppercase tracking-wider flex items-center gap-3">
                      <s.icon size={16} className={s.color} /> {s.label}
                    </span>
                    <span className="text-[var(--text-primary)] font-black">{s.value}</span>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  studentStats.matieres.map((m, i) => (
                    <div key={i} className="space-y-2">
                      <div className="flex justify-between text-[10px] font-bold uppercase tracking-tighter">
                        <span className="text-[var(--text-secondary)]">{m.chapitre__matiere__nom}</span>
                        <span className="text-[var(--text-primary)]">{formatTime(m.total_secondes)}</span>
                      </div>
